...
class CalendarController extends AbstractTwigController
{
    ...
    public function sources_sync(Request $request, Response $response, array $args = []): Response {
        ...
        foreach($calendar->vevent as $event) {
            ...
            $json['dtstart'] = (string)$event->dtstart;
            ...
            $json['group_id'] = hash('sha256','__UID:'.$json['uid'].'__SEQ:'.$json['sequence'].'__RID:'.$json['recurrence_id']);
...
*/
    ...
    public function events_list(Request $request, Response $response, array $args = []): Response {
      ...
      foreach ($result as $obj) {
        $item = json_decode($obj[$key], true);
        $dt = Carbon::now();
        $dtstart = Carbon::parse(strtotime((string)$item['dtstart']));
        $dtend = Carbon::parse(strtotime((string)$item['dtend']));
        /*$events[$uid]['hrcreated'] = $carb_created->diffForHumans($carb_now);
            $events[$uid]['end'] = strtotime($dtend);
            $events[$uid]['created'] = (string)$event->created;
          */  
        $item['duration'] = $dtstart->diffForHumans($dtend, true, true);
        //$item['interval'] = date_diff(date_create($item['dtend']), date_create($item['dtstart']));
        $data[] = $item;
      }
